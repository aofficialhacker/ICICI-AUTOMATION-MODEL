    ...
    if num_match:
        try:
            ...
            if num_val == int(num_val): # for 15, 50, 0
                ...
                if 0 < num_val < 1 and "." in num_match.group(1): # 0.33 -> 33%
                     val_as_percent = num_val * 100
                     if val_as_percent == int(val_as_percent):
                         return f"{int(val_as_percent)}%"
                     else:
            ...
            pass
        if num_match.group(1) == "0": # Should be caught by float conversion now
            return "0%"
    return None
...
def parse_percentage_cell_text(cell_text_original, base_header_details, rto_cluster_from_row, main_table_context_global):
    ...
    for line_idx, line_text_orig in enumerate(cell_lines):
        ...
        explicit_percent_regex = r"(\d+(?:\.\d+)?%)" # Defines the delimiter
        # Corrected splitting logic:
        # Find all percentages and their positions. Then, split the string based on these.
        # This avoids the re.split issue with capturing groups at the start.
        ...
        if not positions: # No percentages found in this line
            ...
            for start, end, po_value in positions:
                # Text before this percentage
                assoc_text = line_text_orig_stripped[current_pos:start].strip()
                all_segments_from_cell_lines.append({"po_percent": po_value, "associated_text": assoc_text})
                current_pos = end
            
            # Any remaining text after the last percentage is a condition line
    ...
    # Fallback if no PO% segments were created by the above logic (e.g., cell is just "0.15")
    ...
    for seg_idx, seg_data_final in enumerate(final_percent_segments_to_process):
        ...
        main_context_makes_to_use_for_this_iteration = [None] # Default
        if main_table_context_global and main_table_context_global.get("bike_makes_main"):
            main_context_makes_to_use_for_this_iteration = main_table_context_global.get("bike_makes_main")
            if not main_context_makes_to_use_for_this_iteration : main_context_makes_to_use_for_this_iteration = [None] # Ensure iteration
        
        print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Before Bike Make Hierarchy, main_context_makes_to_use_for_this_iteration: {main_context_makes_to_use_for_this_iteration}")


        # Apply main table context (from table 2 title) - bike_make is handled per iteration of mcbm
        ...
        if general_conditions_from_cell.get("age_cond"): current_details_for_segment["age"] = general_conditions_from_cell.get("age_cond")
        ...
        print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Processing segment: PO={seg_data_final['po_percent']}, AssocText='{associated_text_segment_orig}'")
        ...
        for age_pattern_re, age_val_resolver in AGE_KEYWORD_PATTERNS:
            ...
            if age_match_seg:
                current_details_for_segment["age"] = age_val_resolver(age_match_seg) if callable(age_val_resolver) else age_val_resolver
                break
        ...
        elif base_header_details.get("header_bike_makes_list"): # If make is in column header
            bike_makes_to_generate_rows_for_this_segment = base_header_details.get("header_bike_makes_list")
        elif main_table_context_global and main_table_context_global.get("bike_makes_main"): # If from main table title (for table 2)
