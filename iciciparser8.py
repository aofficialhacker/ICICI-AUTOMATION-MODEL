...
def extract_explicit_percentage(text_segment):
    match = re.search(r"(\d+(?:\.\d+)?%)", text_segment) # Requires % symbol
    if match:
        return match.group(1)
    # For numbers like "0.15" that should become "0.15%" if they are standalone
    num_match = re.match(r"^\s*(\d+(?:\.\d+)?)\s*$", text_segment) # Match if segment is ONLY a number
    if num_match:
...
def parse_main_table_header(header_text_full):
    ...
    paren_remarks = re.findall(r"\((.*?)\)", text_cleaned_orig) # Use original case for remarks
    for pr in paren_remarks:
        all_remarks_main_header.append(f"({pr})")
    
    # Heuristic to add full original text if it contains more than just parsed keywords
    ...
    for common_word in ["GRID", "ONLY", "AND", "&", "AOTP", "MHCV", "LCV"]: # These are often structural
        temp_remark_check = re.sub(r'\b'+re.escape(common_word)+r'\b',"", temp_remark_check, flags=re.IGNORECASE)
    temp_remark_check = temp_remark_check.replace("(","").replace(")","").strip(" ,.-") # More strip chars

    if len(temp_remark_check) > 3 and temp_remark_check.lower() not in ["aotp", "mhcv", "lcv", "tata", "al"]: # Avoid adding if only noise remains
...
def parse_column_header_text(header_cell_text_original):
    ...
    # Bug 5: Prioritize specific "PCVTAXI" then general "TAXI"
    if "PCVTAXI" in text_upper:
        ...
        if "ELECTRIC" in text_upper: # Check for PCVTAXI_ELECTRIC
            base_details["fuel_type"] = "ELECTRIC"
    elif TAXI_MATCH_REGEX.search(text_upper) and not base_details.get("vehicle"): # If not already set by PCVTAXI
...
def find_special_cluster_in_text(text_upper, is_for_override=True): # Bug 1
    """Finds special cluster codes, prioritizing exact then prefix for override, checking for ONLY/IN context."""
    # Iterate through known codes (sorted by length desc)
    for scc in SPECIAL_CLUSTER_CODES:
        scc_upper = scc # Already upper
        
        # Pattern 1: Exact word match \bSCC\b
        exact_pattern = r"\b" + re.escape(scc_upper) + r"\b"
        exact_match = re.search(exact_pattern, text_upper)
        if exact_match:
            if not is_for_override: return scc_upper # For general detection, context not needed
            # For override, check context
            # Check for "SCC ONLY" or "ONLY SCC" or "IN SCC"
        ...
        # Pattern 2: Concatenated like SCCONLY (e.g., WB1ONLY) - only if for override
        if is_for_override:
            concat_pattern = r"\b(" + re.escape(scc_upper) + r")(ONLY\b|ONLY$)"
            concat_match = re.search(concat_pattern, text_upper)
            if concat_match:
                return concat_match.group(1).upper() # Return the SCC part
...
def parse_percentage_cell_text(cell_text_original, base_header_details, rto_cluster_from_row, main_table_context_global):
    ...
    for line_idx, line_text_orig in enumerate(cell_lines):
        ...
        # Bug 3 Fix: Corrected segmentation for multiple percentages on a line
        # Regex for explicit percentage (e.g., "45%", "55.5%")
        explicit_percent_regex = r"(\d+(?:\.\d+)?%)"
        
        # Split the line by these explicit percentages, keeping the delimiters (percentages)
        # Example: "1-5 yrs 45% >5 yrs 55% only TATA" -> ['1-5 yrs ', '45%', ' >5 yrs ', '55%', ' only TATA']
        line_parts_split = re.split(f'({explicit_percent_regex})', line_text_orig_stripped)
        
        # Filter out empty strings and combine text parts with their subsequent percentage
        # Iterate through parts: if a part is a percentage, the preceding part (if any) is its associated text.
        # And the part after it (if not another percentage) is also associated.
        
        current_segment_text_parts = []
        for k_part_idx, part_str in enumerate(line_parts_split):
            part_stripped = part_str.strip()
            if not part_stripped: continue

            is_this_part_a_percent = extract_explicit_percentage(part_stripped) == part_stripped

            if is_this_part_a_percent:
                # This part is a percentage. The accumulated current_segment_text_parts are its associated text.
                assoc_text = " ".join(current_segment_text_parts).strip()
                all_segments_from_cell_lines.append({
                    "po_percent": part_stripped,
                    "associated_text": assoc_text
                })
                current_segment_text_parts = [] # Reset for next segment
            else:
                # This part is text, add to current segment's associated text
                current_segment_text_parts.append(part_stripped)
        
        # If there's any trailing text after the last percentage on the line, it's a condition line
        if current_segment_text_parts: # Leftover text parts might be conditions
            all_segments_from_cell_lines.append({"po_percent": None, 
                                                 "associated_text": " ".join(current_segment_text_parts).strip(), 
    ...
    if not any(s.get("po_percent") for s in all_segments_from_cell_lines) and cell_text_cleaned_orig_case:
        ...
        if single_po:
             # Associated text is the original cell text minus the extracted percentage
    ...
    for seg in all_segments_from_cell_lines:
        if seg.get("is_condition_line"):
            ...
            if "TATA" in cond_text_upper and "ONLY" in cond_text_upper: # Specific TATA ONLY from condition line
    ...
    for mcbm in main_context_bike_makes_list:
        ...
        for seg_data_final in final_percent_segments_to_process:
            ...
            found_scc_in_segment = find_special_cluster_in_text(associated_text_segment_upper, is_for_override=True) # Bug 1
            if found_scc_in_segment:
                current_details["cluster_code"] = found_scc_in_segment

            bike_makes_to_iterate_for_final_entry = [None]
            
            # "TATA only" rule for the current cell content or segment content
            # This check is specific to the segment's associated text
            ...
            if cell_is_tata_only_special_case or segment_is_tata_only:
                current_details["bike_make"] = "TATA" # Apply TATA make
                bike_makes_to_iterate_for_final_entry = ["TATA"] # Ensure only TATA is processed
            else:
                ...
                if valid_segment_bike_makes_final:
                    ...
                    bike_makes_to_iterate_for_final_entry = [general_conditions_from_cell.get("bike_make_cond")]
            ...
            for bm_final_iter_val in bike_makes_to_iterate_for_final_entry:
                ...
                if (cell_is_tata_only_special_case or segment_is_tata_only) and final_entry["bike_make"] != "TATA":
                ...
                if cell_text_cleaned_orig_case:
                    is_simple_val_for_remark = (cell_text_cleaned_orig_case == final_entry["po_percent"]) or \
                                         (cell_text_cleaned_upper in non_data_values)
