        ...
        try:
            ...
            if 0 < num_val <=1 and "%" not in num_match.group(1) and "." in num_match.group(1) : # 0.15
                 ...
                 if val_as_percent == int(val_as_percent): # 15.0
                     ...
                     return f"{val_as_percent:.1f}%".rstrip('0').rstrip('.') + "%" if '.' in f"{val_as_percent:.1f}%" else f"{val_as_percent:.0f}%"


            if num_val == int(num_val): # for 15, 50, 0
                return f"{int(num_val)}%"
            else: # for 15.5, 0.33
                # if it's less than 1 and has a decimal, it might be 0.33 -> 33%
...
def parse_percentage_cell_text(cell_text_original, base_header_details, rto_cluster_from_row, main_table_context_global):
    ...
    for line_idx, line_text_orig in enumerate(cell_lines):
        ...
        if not positions:
            ...
            for start, end, po_value in positions:
                ...
                # Prevent adding an identical segment if positions somehow yield duplicates for the same text part
                new_segment = {"po_percent": po_value, "associated_text": assoc_text}
                if not all_segments_from_cell_lines or all_segments_from_cell_lines[-1] != new_segment :
                    all_segments_from_cell_lines.append(new_segment)
                else:
                    print(f"DEBUG: parse_percentage_cell_text: Skipping identical segment: {new_segment}")
    ...
    for seg_idx, seg_data_final in enumerate(final_percent_segments_to_process):
        ...
        # Apply main table context (from table 2 title) - bike_make is handled below
        if main_table_context_global:
            ...
            if main_table_context_global.get("age_main"): current_details_for_segment["age"] = main_table_context_global.get("age_main")
        ...
        # Apply general conditions from cell (these might be overridden by segment text)
        if general_conditions_from_cell.get("age_cond"):
            current_details_for_segment["age"] = general_conditions_from_cell.get("age_cond")
            print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Age from general_conditions_from_cell: {current_details_for_segment['age']}")
        ...
        print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Initial age before segment text parse: {current_details_for_segment.get('age')}")
        ...
        # Parse details from the current segment's associated text (this WILL OVERRIDE previous values if found)
        for age_pattern_re, age_val_resolver in AGE_KEYWORD_PATTERNS:
            ...
            if age_match_seg:
                ...
                if resolved_age:
                    ...
                    print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Age OVERRIDDEN by segment text '{associated_text_segment_upper}' -> '{resolved_age}'")
                break
        print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Age AFTER segment text processing: {current_details_for_segment.get('age')}")
        ...
        # Determine the final list of bike makes to iterate over for THIS segment
        ...
        elif base_header_details.get("header_bike_makes_list"): # From column header (takes precedence over main table title IF main table title also exists)
            bike_makes_to_generate_rows_for_this_segment = base_header_details.get("header_bike_makes_list")
        elif main_table_context_global and main_table_context_global.get("bike_makes_main"): # From main table title (Table 2 context)
