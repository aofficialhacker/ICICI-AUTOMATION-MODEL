...
def parse_percentage_cell_text(cell_text_original, base_header_details, rto_cluster_from_row, main_table_context_global):
    ...
    is_target_cell_plan_type = "AOTP" in cell_text_cleaned_upper and "COMP" in cell_text_cleaned_upper # For targeted debugging
    ...
    if is_target_cell_plan_type:
        ...
        print(f"DETAILED DEBUG: Initial base_header_details: {base_header_details}")
        print(f"DETAILED DEBUG: Main table context global: {main_table_context_global}")
    ...
    for line_idx, line_text_orig in enumerate(cell_lines):
        ...
        # Regex to find either "percentage then optional keywords" or "keywords then percentage"
        # This pattern tries to capture a percentage and any immediately following non-percentage text before a comma or end of line.
        # Or text before a percentage.
        # Pattern: (optional text)(percentage)(optional text before comma/end)
        # We are looking for blocks like: "58% AOTP", "40% COMP" or "TATA 45%"
        # This will be complex to do with a single regex split. Let's iterate using finditer.

        # Find all percentages and their start/end positions
        po_matches = list(re.finditer(explicit_percent_regex_str, line_text_orig_stripped))
        if is_target_cell_plan_type: print(f"DETAILED DEBUG: Line '{line_text_orig_stripped}', Found PO matches: {[(m.group(1), m.start(), m.end()) for m in po_matches]}")


        if not po_matches: # No percentages on this line, treat as condition
            all_segments_from_cell_lines.append({"po_percent": None, "associated_text": line_text_orig_stripped, "is_condition_line": True})
            continue

        last_idx_processed = 0
        for i, current_po_match in enumerate(po_matches):
            po_value = current_po_match.group(1)
            po_start = current_po_match.start()
            po_end = current_po_match.end()

            # Text BEFORE the current percentage (from end of last PO or start of line)
            text_before_this_po = line_text_orig_stripped[last_idx_processed:po_start].strip()

            # Determine the end of context for THIS percentage
            # It's either before the next percentage, or end of line
            context_end_for_this_po = len(line_text_orig_stripped)
            if i + 1 < len(po_matches): # If there is a next PO
                next_po_start = po_matches[i+1].start()
                context_end_for_this_po = next_po_start
            
            # Text AFTER this PO up to the next PO or end of line
            text_after_this_po = line_text_orig_stripped[po_end:context_end_for_this_po].strip()
            
            # Smart associated text:
            # If text_after_this_po contains a plan type, it's likely "PO% PLAN_TYPE"
            # If text_before_this_po contains a plan type, it's likely "PLAN_TYPE PO%"
            # Otherwise, combine them, or prioritize based on heuristics.

            assoc_text_for_segment = ""
            plan_type_match_after = PLAN_TYPE_REGEX.search(text_after_this_po)
            plan_type_match_before = PLAN_TYPE_REGEX.search(text_before_this_po)

            if plan_type_match_after:
                # Extract the plan type and any text immediately with it from 'text_after_this_po'
                # e.g., "AOTP," -> "AOTP" ; "COMP (city)" -> "COMP (city)"
                # We might need to take text up to the first comma or significant delimiter if multiple conditions follow.
                potential_assoc_after = text_after_this_po.split(",")[0].strip() # Simple split for now
                assoc_text_for_segment = potential_assoc_after
            elif plan_type_match_before:
                assoc_text_for_segment = text_before_this_po
            else: # No plan type found immediately before or after, combine surrounding text
                assoc_text_for_segment = (text_before_this_po + " " + text_after_this_po).strip()
            
            # If associated text became empty but original parts were not, use original parts for remark
            if not assoc_text_for_segment.strip() and (text_before_this_po or text_after_this_po):
                assoc_text_for_segment = (text_before_this_po + " " + text_after_this_po).strip()


            all_segments_from_cell_lines.append({"po_percent": po_value, "associated_text": assoc_text_for_segment})
            if is_target_cell_plan_type: print(f"DETAILED DEBUG: Segment created: PO={po_value}, AssocText='{assoc_text_for_segment}' (from before='{text_before_this_po}', after='{text_after_this_po}')")
            
            last_idx_processed = po_end + len(text_after_this_po) # Move pointer past the consumed context

        # Any remaining text after the last PO and its context on the line
        if last_idx_processed < len(line_text_orig_stripped):
            remaining_text_on_line = line_text_orig_stripped[last_idx_processed:].strip()
            if remaining_text_on_line:
                all_segments_from_cell_lines.append({"po_percent": None, "associated_text": remaining_text_on_line, "is_condition_line": True})
                if is_target_cell_plan_type: print(f"DETAILED DEBUG: Remaining text on line added as condition: '{remaining_text_on_line}'")

    if is_target_cell_plan_type: print(f"DETAILED DEBUG: all_segments_from_cell_lines after refined splitting: {all_segments_from_cell_lines}")
    ...
    for seg_idx_pre, seg_pre in enumerate(all_segments_from_cell_lines):
        if seg_pre.get("is_condition_line"):
            ...
            # Check for plan type in general condition lines too
    ...
    for seg_idx, seg_data_final in enumerate(final_percent_segments_to_process):
        ...
        if main_table_context_global:
            ...
            if main_table_context_global.get("plan_type_main") and not current_details_for_segment.get("plan_type") : # Apply if not set by column
        ...
        # Apply general plan type condition if no plan type yet
        ...
        if general_conditions_from_cell.get("cond_line_remark_list"):
            current_remarks_list_for_segment.extend(general_conditions_from_cell.get("cond_line_remark_list"))
        ...
        elif not current_details_for_segment.get("plan_type") and general_conditions_from_cell.get("plan_type_cond"): # Check again after segment text processing
             current_details_for_segment["plan_type"] = general_conditions_from_cell.get("plan_type_cond")
             if is_target_cell_plan_type: print(f"DETAILED DEBUG (seg {seg_idx}): Plan type from general_cond (fallback): {current_details_for_segment['plan_type']}")
        ...
        for bm_to_apply in bike_makes_to_generate_rows_for_this_segment:
            ...
            if is_target_cell_plan_type: print(f"DETAILED DEBUG (seg {seg_idx}) Appended final entry: {final_entry} (bm_to_apply: {bm_to_apply})")

    if is_target_cell_plan_type: print(f"DETAILED DEBUG: END - Total results for cell '{cell_text_original}' = {len(results)}")
    return results
