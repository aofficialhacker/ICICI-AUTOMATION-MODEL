...
AGE_KEYWORD_PATTERNS = [
    ...
    # MODIFIED THIS PATTERN - REMOVED LEADING \b
...
def parse_percentage_cell_text(cell_text_original, base_header_details, rto_cluster_from_row, main_table_context_global):
    ...
    print(f"DEBUG: parse_percentage_cell_text: START - Parsing cell text: '{cell_text_original}' (Cleaned Upper: '{cell_text_cleaned_upper}') for RTO: {rto_cluster_from_row}")
    # print(f"DEBUG: parse_percentage_cell_text: Initial base_header_details: {base_header_details}")
    # print(f"DEBUG: parse_percentage_cell_text: Main table context global: {main_table_context_global}")
    ...
    if cell_text_cleaned_upper in non_data_values or not cell_text_cleaned_upper :
        ...
        # print(f"DEBUG: parse_percentage_cell_text: Cell is non-data value ('{cell_text_original}'). Created {len(results)} basic entries: {results}")
        return results
    ...
    for line_idx, line_text_orig in enumerate(cell_lines):
        ...
        explicit_percent_regex = r"(\d+(?:\.\d+)?%)"
        
        positions = []
        for match in re.finditer(explicit_percent_regex, line_text_orig_stripped):
            positions.append((match.start(), match.end(), match.group(1)))
        
        # print(f"DEBUG: parse_percentage_cell_text: Line '{line_text_orig_stripped}', Found POs at positions: {positions}")

        current_pos = 0
        if not positions:
            all_segments_from_cell_lines.append({"po_percent": None, "associated_text": line_text_orig_stripped, "is_condition_line": True})
        else:
            for start, end, po_value in positions:
                assoc_text = line_text_orig_stripped[current_pos:start].strip()
                new_segment = {"po_percent": po_value, "associated_text": assoc_text}
                is_duplicate_po_segment = False
                if all_segments_from_cell_lines and \
                   all_segments_from_cell_lines[-1]["po_percent"] == po_value and \
                   all_segments_from_cell_lines[-1]["associated_text"] == assoc_text:
                    is_duplicate_po_segment = True
                
                if not is_duplicate_po_segment:
                     all_segments_from_cell_lines.append(new_segment)
                # else:
                    # print(f"DEBUG: parse_percentage_cell_text: Skipping identical PO segment: {new_segment}")
                current_pos = end
            
            remaining_text = line_text_orig_stripped[current_pos:].strip()
            if remaining_text:
                 all_segments_from_cell_lines.append({"po_percent": None, "associated_text": remaining_text, "is_condition_line": True})

    # print(f"DEBUG: parse_percentage_cell_text: all_segments_from_cell_lines after line splitting & segmenting: {all_segments_from_cell_lines}")


    if not any(s.get("po_percent") for s in all_segments_from_cell_lines) and cell_text_cleaned_orig_case:
        # print(f"DEBUG: parse_percentage_cell_text: No PO% in segments from line splitting, trying full cell parse for '{cell_text_cleaned_orig_case}'")
        ...
        if single_po:
             ...
             # print(f"DEBUG: parse_percentage_cell_text: Found single PO from full cell: {all_segments_from_cell_lines}")
             ...
             # print(f"DEBUG: parse_percentage_cell_text: Full cell treated as condition: {all_segments_from_cell_lines}")
    ...
    for seg_idx_pre, seg_pre in enumerate(all_segments_from_cell_lines):
        # print(f"DEBUG: parse_percentage_cell_text: Pre-processing segment {seg_idx_pre}: {seg_pre}")
        if seg_pre.get("is_condition_line"):
            ...
            if seg_pre.get("po_percent", "").upper().replace("%", "") in [r.upper().replace("RTO","").strip().replace("RTOS","").strip() for r in SPECIAL_CLUSTER_CODES + [rto_cluster_from_row]]:
                # print(f"DEBUG: Skipping segment with RTO name as po_percent: {seg_pre.get('po_percent')}")
                continue
            final_percent_segments_to_process.append(seg_pre)
    
    # print(f"DEBUG: parse_percentage_cell_text: final_percent_segments_to_process: {final_percent_segments_to_process}")
    # print(f"DEBUG: parse_percentage_cell_text: general_conditions_from_cell: {general_conditions_from_cell}")
    ...
    if not final_percent_segments_to_process and general_conditions_from_cell:
         ...
         # print(f"DEBUG: parse_percentage_cell_text: Added dummy segment for general conditions. final_percent_segments_to_process: {final_percent_segments_to_process}")


    for seg_idx, seg_data_final in enumerate(final_percent_segments_to_process):
        # print(f"DEBUG: parse_percentage_cell_text: SEGMENT PROCESSING LOOP - seg_idx={seg_idx}, seg_data_final={seg_data_final}")
        ...
        if main_table_context_global:
            # print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Applying main_table_context_global: {main_table_context_global} to segment details")
            ...
            if main_table_context_global.get("plan_type_main"): current_details_for_segment["plan_type"] = main_table_context_global.get("plan_type_main")
        ...
        # print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Initial age before segment text parse: {current_details_for_segment.get('age')} (was: {initial_age_for_segment})")
        ...
        for age_pattern_idx, (age_pattern_re, age_val_resolver) in enumerate(AGE_KEYWORD_PATTERNS):
            # Specific debug for the problematic case if assoc text is '>5 YRS'
            if associated_text_segment_upper == '>5 YRS':
                 print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}, assoc: '{associated_text_segment_upper}') Testing age pattern {age_pattern_idx}: '{age_pattern_re.pattern}'")
            
            age_match_seg = age_pattern_re.search(associated_text_segment_upper)
            if age_match_seg:
                if associated_text_segment_upper == '>5 YRS': # More debug for this specific case
                     print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}, assoc: '{associated_text_segment_upper}') MATCHED age pattern {age_pattern_idx}: '{age_pattern_re.pattern}'")
                
                resolved_age = age_val_resolver(age_match_seg) if callable(age_val_resolver) else age_val_resolver
                print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}, assoc: '{associated_text_segment_upper}') Age pattern matched. Resolved age initially: '{resolved_age}'")
                if resolved_age is not None and resolved_age != "":
                    ...
                    print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}, assoc: '{associated_text_segment_upper}') Age OVERRIDDEN by segment text to: '{resolved_age}'")
                else:
                    print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}, assoc: '{associated_text_segment_upper}') Age pattern matched but resolved_age was empty/None.")
                break 
        
        # print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Age AFTER segment text processing: {current_details_for_segment.get('age')}, age_explicitly_set_by_segment: {age_explicitly_set_by_segment}")


        plan_type_match_seg = PLAN_TYPE_REGEX.search(associated_text_segment_upper)
        if plan_type_match_seg: current_details_for_segment["plan_type"] = PLAN_TYPE_KEYWORDS.get(plan_type_match_seg.group(1).upper())
        ...
        # print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Bike make determination: cell_tata_only={cell_is_tata_only_special_case}, seg_tata_only={segment_is_tata_only}, valid_seg_makes={valid_segment_makes}, gen_cond_make={general_conditions_from_cell.get('bike_make_cond')}, main_table_makes={main_table_context_global.get('bike_makes_main') if main_table_context_global else None}, header_makes={base_header_details.get('header_bike_makes_list')}")
        ...
        # print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) bike_makes_to_generate_rows_for_this_segment = {bike_makes_to_generate_rows_for_this_segment}")
        ...
        for bm_to_apply in bike_makes_to_generate_rows_for_this_segment:
            ...
            # print(f"DEBUG: parse_percentage_cell_text: (seg {seg_idx}) Appended final entry: {final_entry} (bm_to_apply: {bm_to_apply})")

    # print(f"DEBUG: parse_percentage_cell_text: END - Total results for cell '{cell_text_original}' = {len(results)}")
    return results
