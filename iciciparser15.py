...
def parse_percentage_cell_text(cell_text_original, base_header_details, rto_cluster_from_row, main_table_context_global):
        ...
        if not po_matches:
            ...
            for i, current_po_match in enumerate(po_matches):
                ...
                if (not text_before_this_po or re.fullmatch(r"[,\s]*", text_before_this_po)) or \
                   PLAN_TYPE_REGEX.search(first_chunk_after_po.upper()): # If text_before is weak OR text_after has a plan type
                ...
                # Advance last_processed_char_idx_on_line to the end of the text chunk used for this PO's context
                if assoc_text_for_segment == first_chunk_after_po and text_immediately_after_po: # if text_after was chosen
                    last_processed_char_idx_on_line = po_end + text_immediately_after_po.find(first_chunk_after_po) + len(first_chunk_after_po)
                    # Add 1 if a comma was part of what was split to move past it
                    if "," in text_immediately_after_po and text_immediately_after_po.startswith(first_chunk_after_po + ","):
                         last_processed_char_idx_on_line +=1
                else: # if text_before was chosen, or text_after was empty
    ...
    for seg_idx, seg_data_final in enumerate(final_percent_segments_to_process):
        ...
        # Apply age from general conditions first (can be overridden by segment text)
        if general_conditions_from_cell.get("age_cond"):
            current_details_for_segment["age"] = general_conditions_from_cell.get("age_cond")
        
        if is_target_cell_age and seg_data_final.get('associated_text','').upper() == '>5 YRS': # Focus debug
             print(f"DETAILED DEBUG (seg {seg_idx}): Age before segment text parse: {current_details_for_segment.get('age')}")
        ...
        # --- Attribute parsing from associated_text_segment_upper ---
        age_explicitly_set_by_segment = False
        for age_pattern_idx, (age_pattern_re, age_val_resolver) in enumerate(AGE_KEYWORD_PATTERNS):
            if is_target_cell_age and associated_text_segment_upper == '>5 YRS': # Focus debug
            ...
            if age_match_seg:
                if is_target_cell_age and associated_text_segment_upper == '>5 YRS': # Focus debug
                     print(f"DETAILED DEBUG (seg {seg_idx}, assoc: '{associated_text_segment_upper}') MATCHED age pattern {age_pattern_idx}: '{age_pattern_re.pattern}' with groups: {age_match_seg.groups() if age_match_seg.groups() else age_match_seg.group(0)}")
