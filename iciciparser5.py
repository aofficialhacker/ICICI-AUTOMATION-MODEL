...
def extract_explicit_percentage(text_segment):
    ...
    # For numbers like "0.15" that should become "0.15%"
    # Ensure it's a number and not part of e.g. an age range "1-5"
...
def parse_main_table_header(header_text_full):
    ...
    if context.get("age_main"):
        # Crude removal for remark check
        ...
        if age_keyword_for_re: # If anything left after removing units
    ...
    # Remove common structural words before deciding if the rest is a remark
    for common_word in ["GRID", "ONLY", "AND", "&", "AOTP", "MHCV", "LCV"]: # Add more if needed
...
def parse_column_header_text(header_cell_text_original):
    ...
    if TAXI_MATCH_REGEX.search(text_upper): # Bug 5: Check for TAXI
        if not (exclusion_triggered and "TAXI" in base_details["excluded_vehicles_col_header"]):
            base_details["vehicle"] = "TAXI"
            if not base_details.get("veh_type") or base_details.get("veh_type") != "PCV": # Check if already set by PCVTAXI
    ...
    if specific_vehicle_matches_raw:
        for sv_match_raw_tuple in specific_vehicle_matches_raw: # findall might return tuples
    ...
    if header_bike_makes_found:
        for hbm_found_upper_tuple in header_bike_makes_found: # findall returns tuples if regex has groups
            hbm_found_upper = hbm_found_upper_tuple if isinstance(hbm_found_upper_tuple, str) else hbm_found_upper_tuple[0]
            hbm_found_upper = hbm_found_upper.upper()
...
def parse_percentage_cell_text(cell_text_original, base_header_details, rto_cluster_from_row, main_table_context_global):
    ...
    # Split by newline, then process each line for segments
    cell_lines = cell_text_cleaned_orig_case.splitlines() if '\n' in cell_text_cleaned_orig_case else [cell_text_cleaned_orig_case]

    for line_text_orig in cell_lines:
        line_text_orig_stripped = line_text_orig.strip()
        if not line_text_orig_stripped: continue
        print(f"DEBUG: parse_percentage_cell_text: Processing Line: '{line_text_orig_stripped}'")

        # Bug 3 Fix: More robust segmentation of a line
        # Find all EXPLICIT percentages (e.g., "45%", "55.5%")
        # The regex r"(\d+(?:\.\d+)?%)" requires the % symbol.
        percent_matches = list(re.finditer(r"(\d+(?:\.\d+)?%)", line_text_orig_stripped))

        if not percent_matches:
            # This line has no explicit percentages, treat as a condition-only line
        ...
        for i, match in enumerate(percent_matches):
            ...
            # Associated text for this percentage is:
            # (text from last_text_end_idx to percent_start_idx) + 
            # (text from percent_end_idx to start of next percent or end of line)
            
            # Text before current %
            text_before = line_text_orig_stripped[last_text_end_idx:percent_start_idx].strip()

            # Text after current % until next % or EOL
            next_percent_marker_start = len(line_text_orig_stripped) # Default to EOL
            if i + 1 < len(percent_matches):
                next_percent_marker_start = percent_matches[i+1].start()
            
            text_after = line_text_orig_stripped[percent_end_idx:next_percent_marker_start].strip()
            
            # Combine text before and after, ensuring no double spacing if one is empty
            current_associated_text = (text_before + " " + text_after).strip()
            
            all_segments_from_cell_lines.append({
                "po_percent": percent_val,
                "associated_text": current_associated_text
            })
            last_text_end_idx = next_percent_marker_start # for the text *before* the *next* percentage

    # If after line processing, no percentage segments were found, but the original cell had text
    # (e.g. "0.15" without a '%', or just "TATA only")
    if not any(s.get("po_percent") for s in all_segments_from_cell_lines) and cell_text_cleaned_orig_case:
        # Try to see if the whole cell is a percentage-like number
        single_po = extract_explicit_percentage(cell_text_cleaned_orig_case) # This now handles numbers like "0.15" -> "0.15%"
        if single_po:
            all_segments_from_cell_lines.append({"po_percent": single_po, "associated_text": ""})
        else: # Treat as a condition line if not a clear percentage
    ...
    for seg in all_segments_from_cell_lines:
        if seg.get("is_condition_line"):
            ...
            if not general_conditions_from_cell.get("bike_make_cond"):
                bm_cond_tuple = BIKE_MAKE_REGEX.findall(cond_text_upper) # findall returns list of tuples if regex has groups
            ...
            if seg.get("po_percent", "").upper().replace("%", "") in [r.upper().replace("RTO","").strip() for r in SPECIAL_CLUSTER_CODES + [rto_cluster_from_row]]:
    ...
    if not final_percent_segments_to_process and general_conditions_from_cell: # Only conditions, no %
        # If there was a condition line but no actual percentage, create an entry with empty PO
        # and apply the conditions. This might happen if a cell is just "TATA Only".
         final_percent_segments_to_process.append({"po_percent": "", "associated_text": ""}) # Create one segment to apply conditions
    ...
    for mcbm in main_context_bike_makes_list:
        ...
        for seg_data_final in final_percent_segments_to_process:
            ...
            if associated_text_segment_orig: current_remarks_list.append(associated_text_segment_orig) # Add the direct associated text
            
            age_override_found_in_segment = False
            for age_pattern_re, age_val_resolver in AGE_KEYWORD_PATTERNS: # Bug 4 Age Hierarchy
            ...
            # Cluster code from segment (Bug 1)
            ...
            elif general_conditions_from_cell.get("cluster_code_cond"): # Fallback to general cell line cluster code
                 current_details["cluster_code"] = general_conditions_from_cell.get("cluster_code_cond")


            # Bike Make Logic (Bug 4 Hierarchy)
            # Priority: 1. Segment Text, 2. General Cell Condition Line, 3. Column Header, 4. Main Table Context
            ...
            bike_makes_to_iterate_for_final_entry = [None] # Default

            if valid_segment_bike_makes_final:
                bike_makes_to_iterate_for_final_entry = valid_segment_bike_makes_final
            elif general_conditions_from_cell.get("bike_make_cond"): # Make from condition line within cell
            ...
            elif mcbm: # Make from main table context (e.g. TATA or AL for second table)
            ...
            for bm_final_iter_val in bike_makes_to_iterate_for_final_entry:
                ...
                # Bug 2: Refine "(Full Cell...)" to be less noisy or more conditional
                if cell_text_cleaned_orig_case:
                    # Add if it's complex or not fully captured by segment-specific remarks
                    # This is heuristic. Let's keep it for now if it helps debug unmatched conditions.
                    if cell_text_cleaned_orig_case not in final_remarks_for_entry: # Avoid exact duplicates
...
def process_sheet(df_sheet, sheet_name):
    ...
    for i_scan_t2_header in range(header_row_idx_t1 + 1, len(df_sheet)):
        ...
        # More robust check for "RTO CLUSTER" considering it might not be the only text
            ...
            for j_scan_title_t2 in range(title_search_start_row_t2, header_row_idx_t2): 
                ...
                for k_col_title_t2 in scan_cols_for_title_t2:
                    if j_scan_title_t2 < len(df_sheet) and k_col_title_t2 < df_sheet.shape[1]:
                        ...
                        if "GRID" in cell_text_title_t2_upper and ("MHCV" in cell_text_title_t2_upper or "LCV" in cell_text_title_t2_upper or "AOTP" in cell_text_title_t2_upper or "TATA & AL ONLY" in cell_text_title_t2_upper) : # Bug 7: Ensure title is picked
