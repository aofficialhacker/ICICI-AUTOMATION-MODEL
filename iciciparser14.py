...
def extract_explicit_percentage(text_segment):
    ...
    if num_match:
        try:
            ...
            if 0 < num_val <=1 and "%" not in num_match.group(1) and "." in num_match.group(1) :
...
def parse_percentage_cell_text(cell_text_original, base_header_details, rto_cluster_from_row, main_table_context_global):
    ...
    explicit_percent_regex_str = r"(\d+(?:\.\d+)?%)" # Defined here
    ...
    if is_target_cell_plan_type or is_target_cell_age: # Focused debug
    ...
    for line_idx, line_text_orig in enumerate(cell_lines):
        ...
        # Find all percentages and their start/end positions
        po_matches = []
        for match in re.finditer(explicit_percent_regex_str, line_text_orig_stripped): # Use explicit_percent_regex_str
            po_matches.append({'value': match.group(1), 'start': match.start(), 'end': match.end()})
        
        if is_target_cell_plan_type or is_target_cell_age:
            print(f"DETAILED DEBUG: Line '{line_text_orig_stripped}', Found PO matches: {po_matches}")

        last_po_end_on_line = 0 
        if not po_matches:
            ...
            for i, current_po_match in enumerate(po_matches):
                ...
                text_before_this_po = line_text_orig_stripped[last_po_end_on_line:po_start].strip()
                
                context_window_end_for_this_po = len(line_text_orig_stripped)
                if i + 1 < len(po_matches):
                    next_po_start_on_line = po_matches[i+1]['start'] # Corrected here
                ...
                assoc_text_for_segment = text_before_this_po # Default to text before
                
                # If text_before is empty or just punctuation, OR if text_after contains a plan type,
                # then text_after (up to comma) becomes the primary association.
                if (not text_before_this_po or re.fullmatch(r"[,\s]*", text_before_this_po)) or \
                   PLAN_TYPE_REGEX.search(text_immediately_after_po.upper()):
                    assoc_text_for_segment = text_immediately_after_po.split(",")[0].strip()

                # If after all that, assoc_text is empty but text_before_this_po was not, revert to text_before_this_po
                if not assoc_text_for_segment.strip() and text_before_this_po:
                ...
                if is_target_cell_plan_type or is_target_cell_age:
                     print(f"DETAILED DEBUG: Segment created: PO='{po_value}', AssocText='{assoc_text_for_segment}' (from before='{text_before_this_po}', after_full_context='{text_immediately_after_po}')")
                
                last_po_end_on_line = po_end # Base advance: end of current PO
                # If text_immediately_after_po was used as assoc_text, advance further
                if assoc_text_for_segment == text_immediately_after_po.split(",")[0].strip() and text_immediately_after_po:
                    consumed_part = text_immediately_after_po.split(",")[0]
                    last_po_end_on_line = po_end + len(consumed_part) + (1 if "," in text_immediately_after_po and text_immediately_after_po.startswith(consumed_part + ",") else 0)


            remaining_text_at_line_end = line_text_orig_stripped[last_po_end_on_line:].strip()
            if remaining_text_at_line_end:
                ...
                if is_target_cell_plan_type or is_target_cell_age: print(f"DETAILED DEBUG: Remaining text on line added as condition: '{remaining_text_at_line_end}'")
    ...
    if not final_percent_segments_to_process and general_conditions_from_cell:
        ...
        if any(k not in ['cond_line_remark_list'] for k in general_conditions_from_cell):
            ...
            # Add dummy if remarks are not just the cell text itself (i.e., some keywords were parsed into general conditions)
    ...
    for seg_idx, seg_data_final in enumerate(final_percent_segments_to_process):
        ...
        if main_table_context_global:
            ...
            if main_table_context_global.get("age_main") and not current_details_for_segment.get("age"):
                 current_details_for_segment["age"] = main_table_context_global.get("age_main")
            if main_table_context_global.get("plan_type_main") and not current_details_for_segment.get("plan_type") :
                 current_details_for_segment["plan_type"] = main_table_context_global.get("plan_type_main")
        
        initial_age_for_segment = current_details_for_segment.get("age")
        if general_conditions_from_cell.get("age_cond"):
            current_details_for_segment["age"] = general_conditions_from_cell.get("age_cond")
        
        if is_target_cell_age and seg_data_final.get('associated_text','').upper() == '>5 YRS':
             print(f"DETAILED DEBUG (seg {seg_idx}): Initial age before segment text parse: {current_details_for_segment.get('age')} (was: {initial_age_for_segment})")
        ...
        if not current_details_for_segment.get("plan_type") and general_conditions_from_cell.get("plan_type_cond"):
            current_details_for_segment["plan_type"] = general_conditions_from_cell.get("plan_type_cond")
            if is_target_cell_plan_type: print(f"DETAILED DEBUG (seg {seg_idx}): Plan type from general_cond (before segment assoc text): {current_details_for_segment['plan_type']}")
        ...
        for age_pattern_idx, (age_pattern_re, age_val_resolver) in enumerate(AGE_KEYWORD_PATTERNS):
            if is_target_cell_age and associated_text_segment_upper == '>5 YRS':
                 print(f"DETAILED DEBUG (seg {seg_idx}, assoc: '{associated_text_segment_upper}') Testing age pattern {age_pattern_idx}: '{age_pattern_re.pattern}'")
            
            age_match_seg = age_pattern_re.search(associated_text_segment_upper)
            if age_match_seg:
                if is_target_cell_age and associated_text_segment_upper == '>5 YRS':
                     print(f"DETAILED DEBUG (seg {seg_idx}, assoc: '{associated_text_segment_upper}') MATCHED age pattern {age_pattern_idx}: '{age_pattern_re.pattern}' with match: {age_match_seg.groups() if age_match_seg.groups() else age_match_seg.group(0)}")

                resolved_age = age_val_resolver(age_match_seg) if callable(age_val_resolver) else age_val_resolver
                
                if is_target_cell_age and associated_text_segment_upper == '>5 YRS':
                     print(f"DETAILED DEBUG (seg {seg_idx}, assoc: '{associated_text_segment_upper}') Resolved age: '{resolved_age}'")
                ...
                if resolved_age is not None and resolved_age != "":
                    ...
                    if is_target_cell_age and associated_text_segment_upper == '>5 YRS':
                         print(f"DETAILED DEBUG (seg {seg_idx}, assoc: '{associated_text_segment_upper}') Age OVERRIDDEN to: '{resolved_age}'")
                break 
        ...
        if plan_type_match_seg:
            current_details_for_segment["plan_type"] = PLAN_TYPE_KEYWORDS.get(plan_type_match_seg.group(1).upper())
            if is_target_cell_plan_type: print(f"DETAILED DEBUG (seg {seg_idx}): Plan type from assoc text '{associated_text_segment_upper}' -> {current_details_for_segment['plan_type']}")
        
        elif not current_details_for_segment.get("plan_type") and general_conditions_from_cell.get("plan_type_cond"):
            current_details_for_segment["plan_type"] = general_conditions_from_cell.get("plan_type_cond")
            if is_target_cell_plan_type: print(f"DETAILED DEBUG (seg {seg_idx}): Plan type from general_cond (final fallback): {current_details_for_segment['plan_type']}")
        ...
        elif main_table_context_global and main_table_context_global.get("bike_makes_main"):
        ...
        for bm_to_apply in bike_makes_to_generate_rows_for_this_segment:
            ...
            if is_target_cell_plan_type or (is_target_cell_age and seg_data_final.get('associated_text','').upper() == '>5 YRS'):
            ...
            for j_col_idx_t2 in range(rto_cluster_col_idx_t2 + 1, len(df_sheet.columns)):
                ...
                for spec_veh_t2 in specific_vehicles_from_header_t2:
                    for ft_t2 in fuel_types_from_header_t2:
                        ...
                        # print(f"DEBUG: process_sheet (Table 2): Passing to parse_percentage_cell_text: base_details={current_base_details_t2_for_iter}, rto='{rto_cluster_val_t2_orig}', cell_value='{cell_value_t2_orig}', main_context={main_table2_context}")
